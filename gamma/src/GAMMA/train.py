...
def train_model(model_defs, input_arg, map_cstr=None, chkpt_file='./chkpt'):
    ...
    for dimension in model_defs:
        ...
        if chkpt is not None:
            best_reward = chkpt['best_reward'][0]
            ...
            print(f"→ Reward: {best_reward:.3e}")
            print(f"  - Runtime   : {best_runtime:.0f} cycles")
            print(f"  - Throughput: {best_throughput:.2f}")
            print(f"  - Energy    : {best_energy:.2f}")
            print(f"  - Power     : {best_power:.2f}")
            print(f"  - Area      : {best_area / 1e6:.3f} mm²")
            print(f"  - Num_PE    : {best_num_pe}")
            print(f"  - L1 Buffer : {best_l1_size} elements")
            print(f"  - L2 Buffer : {best_l2_size} elements")

            # Save final mapping and result files
            final_m_file = f"best_layer{opt.layer}_{opt.model}.m"
            final_csv_file = f"best_layer{opt.layer}_{opt.model}.csv"
            try:
                copyfile(env.last_mapping_file, final_m_file)
                copyfile(env.last_csv_file, final_csv_file)
                print(f"✅ Saved mapping to: {final_m_file}")
                print(f"✅ Saved result to : {final_csv_file}")
            except Exception as e:
                print(f"⚠️ Warning: Could not save mapping files: {e}")
