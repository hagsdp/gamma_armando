...
class GAMMA(object):
    ...
    def mutate_par(self, pop, alpha=0.5):
        ...
        for idx in range(len(pop)):
            if random.random() < alpha:
                # Ensure individual has enough levels (e.g., L3 mapper assumed here)
                if len(pop[idx]) > 7:
                    pop[idx][7][0], pop[idx][0][0] = pop[idx][0][0], pop[idx][7][0]
                    continue  # Perform only swap in this case

                # General fallback mutation if no 3rd level exists
                indv = pop[idx]
                avail_val = len(indv) - 1
                pick = random.randint(0, avail_val)
                pick_level = pick // 7
                ...
                if self.map_cstr and "sp_sz" in self.cstr_list[pick_level]:
                    sp_sz = self.cstr_list[pick_level]["sp_sz"]
                else:
                    if self.fixedCluster < 1:
                        last_cluster_dict = self.scan_indv(indv[:-7]) if pick != 0 else None
                        lastcluster_sz = last_cluster_dict[sp] if last_cluster_dict else self.dimension_dict[sp]
    ...
    def evaluate(self, pool, population, cur_gen=-1):
        ...
        # print(reward_activ_list)

        for i in range(len(population)):
            reward, activity_count = reward_activ_list[i]
            if reward is None: # TODO rethink that: or any(np.array(reward) >= 0):
    ...
    def oberserve_maestro(self, indv, num_pe=None, l1_size=None, l2_size=None, NocBW=None, offchipBW=None):
        ...
        try:
            ...
            # Save the latest valid files before deletion
            # You'll rename them outside based on context
            self.last_mapping_file = f"{m_file}.m"
            self.last_csv_file = f"{m_file}.csv"

            # Comment out auto-deletion
            # os.remove(self.last_mapping_file) if os.path.exists(self.last_mapping_file) else None
            # os.remove(self.last_csv_file) if os.path.exists(self.last_csv_file) else None
